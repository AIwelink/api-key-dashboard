...
- Bug Team 类型识别和容量资格必须分开：显式 `account_type/plan_type=bug_team` 优先级最高；否则仅把 `plan_type=team`、没有有效 5h 窗口且 7d 窗口不少于 28 天的账号识别为 Bug Team。Bug Team 的 7d 使用率低于 100% 时正常计入概览、美元容量和并发；达到 100% 后，恢复时间不超过 1 天才计入动态 5h/7d 容量，超过 1 天或恢复时间未知时排除。临时 403 cooldown 不能替代明确的 7d 100% 判断。
