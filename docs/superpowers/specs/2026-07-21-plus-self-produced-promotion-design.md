...
- HTTP 401 destination: group `7` (`封禁账号池`)
- Probe model: `gpt-5.4`
...
2. If the error represents HTTP 401, including `API returned 401` or a direct test-endpoint 401 response, classify it as `unauthorized_banned` and route it to group `7` without changing its name.
3. If the test returns `success=true`, classify it as `passed`.
4. If the error represents HTTP 429, including text such as `API returned 429` or a direct test-endpoint 429 response, classify it as `rate_limited_but_eligible` and treat the account as successful.
5. All other responses and exceptions are failures. Record the normalized error and leave the remote account unchanged.

HTTP 429 counts as success because it proves the account can reach the requested model even though capacity is temporarily limited. HTTP 401 identifies an unusable credential and routes the account to the banned pool without renaming it. The ChatGPT-account model compatibility error does not prove Plus eligibility and must never promote an account.
...
1. Load the configured Sub2API site with its token and validate that groups `4`, `6`, and `7` exist.
...
- `plus_self_produced_runs`: one summary per execution containing trigger, start/finish timestamps, status, candidate/tested/passed/promoted/banned/failed counts, and a run-level error when setup fails.
...
- Read-only workflow facts for `US06-5002`, Plus flow `4 -> 6`, 401 flow `4 -> 7`, and `gpt-5.4`.
- Last-run metrics for candidates, tested, eligible, promoted, banned, and failed.
...
- A missing site or any missing group (`4`, `6`, or `7`) fails the run before any account mutation.
...
- HTTP 401 account names are never changed.
...
- Classification of `success=true`, HTTP 401, HTTP 429, the ChatGPT-account unsupported-model error, and unrelated errors.
- Unsupported-model classification precedence.
- Exact HTTP 401 group-7 update payload with an unchanged name.
...
- Rendering successful, 401-banned, 429-eligible, unsupported-model, and failed result rows.
