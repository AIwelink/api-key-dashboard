...
- 旧探测快照停止写入，并全量转换为变化批次和每日检查点。
- 转换结果全量校验成功后，按小批次删除旧快照。
...
- 不在迁移脚本中执行 MongoDB `compact` 或其他阻塞性存储维护。
...
- 旧数据归零后再删除旧读取路径和无用索引，单独执行，不与迁移删除绑定。
- 首次运行时，identity 没有 history baseline，则以当前远端动态状态建立 baseline，不产生“所有字段初始化”变化事件。
- 首个每日检查点提供后续历史重建起点。

## 旧快照全量迁移

迁移必须先部署停止旧快照写入的新探测器。脚本启动时读取旧集合最新 `sampled_at`；如果最近 10 分钟仍有新样本，则拒绝进入删除阶段。

迁移以固定的 `source_max_sampled_at` 为边界，按站点和时间顺序流式读取，不一次性载入全部旧集合。内存仅维护每个 identity 的上一份动态状态、累计状态和当前自然日。每个历史 probe run 使用原 `probe_run_id` 生成确定性批次 ID，并增加 `migration_id` 标识。账号首次出现时从空状态生成初始化变化，确保首日也能完整重建。

每个上海自然日使用当天首个完整 probe run 的状态生成每日检查点。旧样本没有订阅信息时只迁移 usage；`cumulative_usage_snapshot` 进入检查点。状态、调度、错误、401 和分组信息不从旧样本重复生成业务事件。

`remote_account_history_migrations` 保存迁移边界、阶段、源文档数、已处理数、变化批次数、检查点数、校验摘要、删除数和错误。转换使用 upsert，可以从头重跑；删除阶段中断后根据固定边界继续删除剩余文档。

删除前执行全量校验：从空状态重放本次迁移的所有变化，逐 identity 比较最终状态 hash；检查每日 manifest 完整性、源处理数量和批次 BSON。任一不一致都将迁移标记为 `verification_failed`，不删除任何旧快照。

校验通过后，每批查询并删除 2,000 个边界内旧快照，逐批更新迁移记录。最终旧集合边界内计数必须为零，迁移状态才可标记为 `completed`。删除释放的 WiredTiger 空间允许 MongoDB 重用；是否执行物理压缩另行评估。
