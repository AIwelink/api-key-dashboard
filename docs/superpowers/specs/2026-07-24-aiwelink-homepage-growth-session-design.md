...
- 全局末次推广访问必须与注册 `site_id` 相同才能形成 `promotion` 来源；不一致时不回退到本站更早的推广链接。
- 无可用推广来源时，服务使用独立主页访问证据分类为 `organic_search` 或 `referral`；仍无证据时分类为 `direct`。这些非推广分类的 `tracking_link_id` 必须为空。
...
| Cookie 缺失或失效 | 正常访问主页和业务站 | 注册成功后分类为 `direct`，不伪造推广证据 |
| 上下文接口失败 | 使用默认 API 入口 | 不向错误站点迁移推广归因 |
| 注册失败 | 显示业务系统原错误 | 不创建用户归因 |
| 已有账号登录 | 正常登录 | 不创建新注册归因 |
| 全局末次推广站点与注册站点不一致 | 正常完成业务注册 | 不回退旧推广链接；按独立主页证据分类，否则归为 `direct` |
...
- 注册绑定唯一约束和幂等；
- 站点不匹配时不形成推广归因、不回退旧链接，并正确分类非推广来源。
