...
const METRIC_HELP_DETAILS: Record<string, MetricHelpDetail> = {
  ...
  "P90 保守可用时间": {
    ...
    formula: "使用未来逐小时P90消耗依次扣减动态可用额度；当前小时结合实时消耗速度，确认上涨后，未来1至3小时再综合本分组历史相似上涨后的延续率进行加权修正。",
    note: "P90追求约90%的真实消耗不超过该边界，因此比P50更保守；它不是最可能发生的时长。短期冲击只按需求状态、样本置信度和历史延续率部分传导，不会完全替代季节预测。",
  },
  "当前速度": {
